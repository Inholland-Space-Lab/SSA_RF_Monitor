...
class I2cLcd(LcdApi):
    ...
    def backlight_blink(self, duty, freq):
        # MODE2 bit5 DMBLNK 1=blinking
        self.i2c.mem_write(32, self.bl_i2c_addr, 1)
        self.i2c.mem_write(255, self.bl_i2c_addr, 8)  # LEDOUT = PWMx + GRPPWM
        # blink
        self.i2c.mem_write(duty, self.bl_i2c_addr, 6)  # GRPPWM = duty cycle
        # GRPFREQ = blinking period (24hz - 10.73s)
        self.i2c.mem_write(freq, self.bl_i2c_addr, 7)

    def backlight_brightness(self, duty):
        # MODE2 bit5 DMBLNK 0=dimming
        self.i2c.mem_write(0, self.bl_i2c_addr, 1)
        self.i2c.mem_write(255, self.bl_i2c_addr, 8)  # LEDOUT = PWMx + GRPPWM
        # set brightness
        # GRPPWM = global brightness
        self.i2c.mem_write(duty, self.bl_i2c_addr, 6)

    def backlight_normal(self):
        self.i2c.mem_write(170, self.bl_i2c_addr, 8)  # LEDOUT = PWMx

    def backlight_sleep(self):
        # MODE1 bit4 SLEEP 1=low power mode, oscillator off
        self.i2c.mem_write(16, self.bl_i2c_addr, 0)

    def backlight_wake(self):
        # MODE1 bit4 SLEEP 0=normal power mode
        self.i2c.mem_write(0, self.bl_i2c_addr, 0)
